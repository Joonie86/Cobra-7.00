...
#define PS2EMU
#define PS2NETEMU
#define FIRMWARE_4_50DEX

#include "../../ps2emu/include/ps2emu/symbols.h"

#define read_iso_size_call	0x135488 //done
#define fstat_iso_call		0x13537C //done
#define open_iso_call1		0x135340 //done
#define open_iso_call2		0x1363F0 //done
#define savedata_patch		0x11A974 //done

#define ADDITIONAL_CODE_SIZE		0xC00 //done
#define ADDITIONAL_DATA_SIZE		0x1000 //done

#define CODE_SECTION_ADDR		0x28E880 //done
#define DATA_SECTION_ADDR		0xB20900 //done
#define PAYLOAD_ADDR		(CODE_SECTION_ADDR+0x78+8) /* CODE_SECTION_ADDR + CODE_SECTION_SIZE + 8 to align to 0x10 */
#define SH_ADDR			0x292B68 /* look in self, not in elf */ //done
...
int main(int argc, char *argv[])
{
	...
	command3("wine scetool.exe", "--decrypt", "temp.self", "temp.elf");	
	...
	command15("wine scetool.exe", "-v", "--sce-type=SELF", "--compress-data=TRUE", "--compress-data=TRUE", "--skip-sections=FALSE", 
		 "--self-auth-id=1020000401000001", "--self-add-shdrs=TRUE", "--self-vendor-id=02000003", "--self-type=LV2", 
	         "--self-fw-version=0004005000000000", "--key-revision=0", "--self-app-version=0004005000000000", "--encrypt", "temp.elf", self_output);
