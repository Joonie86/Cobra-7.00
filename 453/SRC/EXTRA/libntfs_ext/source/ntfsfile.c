...
static size_t ntfs_attr_to_sectors(ntfs_attr *na, const s64 pos, s64 count, uint32_t *sec_out, uint32_t *size_out, int max, uint32_t *s_count)
{
    ...
    for (; count && *s_count < max; rl++, ofs = 0) {
        ...
        if (rl->lcn < (LCN)0) {
            ...
            else
            {
                if (prev_sector == 0xFFFFFFFF)
                {
                    size_out[*s_count-1] += (to_read / 512);
                }
                else
                {
                    prev_sector = sec_out[*s_count] = 0xFFFFFFFF;
                    size_out[*s_count] = to_read / 512;
                    (*s_count)++;
                }
            }
        ...
        if (to_read & 511)
        {
            to_read = to_read + 512 - (to_read % 512);
        }
        
        if (prev_sector == cur_sector)
        {
            size_out[*s_count-1] += (to_read / 512);
            prev_sector += (to_read / 512);
        }
        else
        {
...
int ntfs_file_to_sectors (struct _reent *r, const char *path, uint32_t *sec_out, uint32_t *size_out, int max, int phys) 
{
    ...
    while (len && s_count < max) {
        size_t ret = ntfs_attr_to_sectors(file->data_na, file->pos, len, sec_out, size_out, max, &s_count);
        if (ret <= 0 || ret > len) {
            ntfsUnlock(file->vd);
            r->_errno = errno;
            return -1;
