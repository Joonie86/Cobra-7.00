...
#define PS2EMU
#define PS2NETEMU
#define FIRMWARE_4_53

#include "../../ps2emu/include/ps2emu/symbols.h"

#define read_iso_size_call	0x135500 //
#define fstat_iso_call		0x1353F4 //
#define open_iso_call1		0x1353B8 //
#define open_iso_call2		0x136468 //
#define savedata_patch		0x11A208 //FIXED!!

#define ADDITIONAL_CODE_SIZE		0x6800
#define ADDITIONAL_DATA_SIZE		0x1000

#define CODE_SECTION_ADDR		0x28f5c0 //FIXED!!
#define DATA_SECTION_ADDR		0xb20a00 //FIXED!!
#define PAYLOAD_ADDR		(CODE_SECTION_ADDR+0x78+8) /* CODE_SECTION_ADDR + CODE_SECTION_SIZE + 8 to align to 0x10 */
#define SH_ADDR			0x292348 // /* look in self, not in elf */
...
int main(int argc, char *argv[])
{
	...
	command15("scetool.exe", "-v", "--sce-type=SELF", "--compress-data=TRUE", "--compress-data=TRUE", "--skip-sections=FALSE", 
		 "--self-auth-id=1020000401000001", "--self-add-shdrs=TRUE", "--self-vendor-id=02000003", "--self-type=LV2", 
	         "--self-fw-version=0004003000000000", "--key-revision=0", "--self-app-version=0004005300000000", "--encrypt", "temp.elf", self_output);
